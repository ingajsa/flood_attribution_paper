...
"""This script assignes each basin an overarching trend direction (positive or negative)
   Afterwards the output can be transformed to a grid in 
   /code/scripts_reconstruction/basin_trend_assessment/convert_basins2grid.py
"""


trend_file = '~/data/repro/trends_median_discharge.nc'
...
for bas_idx in range(n_basins):
    ...
    if new_trends.sum() > new_trends_neg.sum():
        dis_reg_geo = 1
    elif new_trends.sum() < exp_pop_neg:
        dis_reg_geo = -1
    else:
        dis_reg_geo = 0
    ...
    df_bas_assig.to_csv('~/data/repro/basin_assignment.csv')
df_bas_assig.to_csv('~/data/repro/basin_assignment.csv')
