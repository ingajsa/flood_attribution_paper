...
def test_residuals(model, timeperiod,reg,dis):
    ...
    fig.savefig('/home/insauer/projects/Attribution/Floods/Paper_NC_Resubmission_data/Response_letter/Autocorr_Residuals/AutocorrResidualsAMO_{}_{}.png'.format(reg,dis),bbox_inches = 'tight',dpi =600)
...
def extract_model_coefs_short(region, model, link, dis):
    ...
    DATA_ATTR.to_csv('/home/insauer/projects/Attribution/Floods/Paper_NC_Resubmission_data/Teleconnections/Lag_PosNeg_ENSO_AMO_PDO_NAO_Loo.csv')     
...
def extract_model_coefs(region, model, link, model10, link10,
    ...
    DATA_ATTR.to_csv('/home/insauer/projects/Attribution/Floods/Paper_NC_Resubmission_data/Teleconnections/Lag_PosNeg_ENSO AMO_PDO_NAO_Loo.csv')
...
for i, test_region in enumerate(test_regions):
    ...
    if test_region != 'NAM':
        ...
        loo_df_pos.to_csv('/home/insauer/projects/Attribution/Floods/Paper_NC_Resubmission_data/Teleconnections/Lag_ENSO_AMO_PDO_NAO_LooIC_{}Pos.csv'.format(test_region))
    ...
    # climateData80Pos = np.array(DATA_region80['NormHaz_ImpFix_2yPos_offset'])
    # climateData80Neg = np.array(DATA_region80['NormHaz_ImpFix_2yNeg_offset'])

    # climateDataPos10 = np.array(DATA_region['NormHaz_Imp2010_2yPos_offset'])
    # climateDataNeg10 = np.array(DATA_region['NormHaz_Imp2010_2yNeg_offset'])

    # climateData8010Pos = np.array(DATA_region80['NormHaz_Imp2010_2yPos_offset'])
    # climateData8010Neg = np.array(DATA_region80['NormHaz_Imp2010_2yNeg_offset'])

    if normClim is True:
        
        climateDataNeg = climateDataNeg/np.nanmax(climateDataNeg)

        # climateData80Pos = climateData80Pos/np.nanmax(climateData80Pos)
        # climateData80Neg = climateData80Neg/np.nanmax(climateData80Neg)

        # climateDataPos10 = climateDataPos10/np.nanmax(climateDataPos10)
        # climateDataNeg10 = climateDataNeg10/np.nanmax(climateDataNeg10)

        # climateData8010Pos = climateData8010Pos/np.nanmax(climateData8010Pos)
        # climateData8010Neg = climateData8010Neg/np.nanmax(climateData8010Neg)
    ...
    loo_df_neg.to_csv('/home/insauer/projects/Attribution/Floods/Paper_NC_Resubmission_data/Teleconnections/Lag_ENSO_AMO_PDO_NAO_LooIC_{}Neg.csv'.format(test_region))

    # best_modelPos10, y10Pos, x10Pos, maxi10Pos,\
    #     pearson_corr10Pos, best_loo10Pos = find_best_model(climateDataPos10, telecon)
    # best_modelNeg10, y10Neg, x10Neg, maxi10Neg,\
    #     pearson_corr10Neg, best_loo10Neg = find_best_model(climateDataNeg10, telecon)

    # best_model80Pos, y80Pos, x80Pos, maxi80Pos,\
    #     pearson_corr80Pos, best_loo80Pos = find_best_model(climateData80Pos, telecon80)
    # best_model80Neg, y80Neg, x80Neg, maxi80Neg,\
    #     pearson_corr80Neg, best_loo80Neg = find_best_model(climateData80Neg, telecon80)

    # best_model8010Pos, y8010Pos, x8010Pos, maxi8010Pos,\
    #     pearson_corr8010Pos, best_loo8010Pos = find_best_model(climateData8010Pos, telecon80)
    # best_model8010Neg, y8010Neg, x8010Neg, maxi8010Neg,\
    #     pearson_corr8010Neg, best_loo8010Neg = find_best_model(climateData8010Neg, telecon80)
    ...
    # coefPos10, pvalPos10 = test_residuals(best_modelPos10, np.arange(1971, 2011))
    # coefNeg10, pvalNeg10 = test_residuals(best_modelNeg10, np.arange(1971, 2011))

    # coef80Pos, pval80Pos = test_residuals(best_model80Pos, np.arange(1980, 2011))
    # coef80Neg, pval80Neg = test_residuals(best_model80Neg, np.arange(1980, 2011))
    # coef8010Pos, pval8010Pos = test_residuals(best_model8010Pos, np.arange(1980, 2011))
    # coef8010Neg, pval8010Neg = test_residuals(best_model8010Neg, np.arange(1980, 2011))

    
    unexTNeg = unexplainable_Trends(pvalNeg, coefNeg, test_region+'_Neg', 'Change H7', 'Sign H7')
    # unexTPos10 = unexplainable_Trends(pvalPos10, coefPos10, test_region+'_Pos',
    #                                   'Change H107', 'Sign H107')
    # unexTNeg10 = unexplainable_Trends(pvalNeg10, coefNeg10, test_region+'_Neg',
    #                                   'Change H107', 'Sign H107')

    # unexT80Pos = unexplainable_Trends(pval80Pos, coef80Pos, test_region+'_Pos',
    #                                   'Change H', 'Sign H')
    # unexT80Neg = unexplainable_Trends(pval80Neg, coef80Neg, test_region+'_Neg',
    #                                   'Change H', 'Sign H')
    # unexT8010Pos = unexplainable_Trends(pval8010Pos, coef8010Pos, test_region+'_Pos',
    #                                     'Change H10', 'Sign H10')
    # unexT8010Neg = unexplainable_Trends(pval8010Neg, coef8010Neg, test_region+'_Neg',
    #                                     'Change H10', 'Sign H10')

    
    DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Res_Sig'] = pvalNeg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Res_Sig10'] = pvalPos10
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Res_Sig10'] = pvalNeg10

    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Res_Sig80'] = pval80Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Res_Sig80'] = pval80Neg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Res_Sig8010'] = pval8010Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Res_Sig8010'] = pval8010Neg

    
    DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Res_Slope'] = coefNeg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Res_Slope10'] = coefPos10
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Res_Slope10'] = coefNeg10

    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Res_Slope80'] = coef80Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Res_Slope80'] = coef80Neg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Res_Slope8010'] = coef8010Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Res_Slope8010'] = coef8010Neg

    
    DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Unexplained Haz'] = unexTNeg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Unexplained Haz10'] = unexTPos10
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Unexplained Haz10'] = unexTNeg10

    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Unexplained Haz80'] = unexT80Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Unexplained Haz80'] = unexT80Neg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Unexplained Haz8010'] = unexT8010Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Unexplained Haz8010'] = unexT8010Neg

    
    DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'CorrCoef'] = pearson_corrNeg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'CorrCoef10'] = pearson_corr10Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'CorrCoef10'] = pearson_corr10Neg

    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'CorrCoef80'] = pearson_corr80Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'CorrCoef80'] = pearson_corr80Neg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'CorrCoef8010'] = pearson_corr8010Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'CorrCoef8010'] = pearson_corr8010Neg
    ...
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'BestLoo10'] = best_loo10Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'BestLoo10'] = best_loo10Neg

    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'BestLoo80'] = best_loo80Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'BestLoo80'] = best_loo80Neg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'BestLoo8010'] = best_loo8010Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'BestLoo8010'] = best_loo8010Neg

    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Best_link'] =\
    #     best_modelPos.fit_history['iteration']
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Rest_link80'] =\
    #     best_modelPos10.fit_history['iteration']
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Best_link10'] =\
    #     best_modelPos10.fit_history['iteration']
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Best_link8010'] =\
    #     best_model8010Pos.fit_history['iteration']

    DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Best_link'] =\
        best_modelNeg.fit_history['iteration']
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Rest_link80'] =\
    #     best_modelNeg10.fit_history['iteration']
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Best_link10'] =\
    #     best_modelPos10.fit_history['iteration']
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Best_link8010'] =\
    #     best_model8010Pos.fit_history['iteration']

    
    DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Link_func'] = yNeg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Link_func10'] = y10Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Link_func10'] = y10Neg

    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Link_func80'] = y80Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Link_func80'] = y80Neg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Link_func8010'] = y8010Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Link_func8010'] = y8010Neg

    
    DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Combi'] = xNeg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Combi10'] = x10Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Combi10'] = x10Neg

    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Combi80'] = x80Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Combi80'] = x80Neg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Combi8010'] = x8010Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Combi8010'] = x8010Neg
    ...
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Maxi10'] = maxi10Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Maxi10'] = maxi10Neg

    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Maxi80'] = maxi80Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Maxi80'] = maxi80Neg
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Pos', 'Maxi8010'] = maxi8010Pos
    # DATA_ATTR.loc[DATA_ATTR['Region'] == test_region+'_Neg', 'Maxi8010'] = maxi8010Neg
